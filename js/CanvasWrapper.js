define([
	...
	var CanvasWrapper = {
		...
		add: function() {
			_canvasOuterWrapper = document.getElementById('canvas-outer-wrapper');
			_canvasInnerWrapper = document.getElementById('canvas-inner-wrapper');
			...
			document.body.removeChild(gooLogo);
			...
			_canvas.addEventListener('mouseup', CanvasWrapper.allowSlection, true);
		...
		uninterruptedPan: function() {
			...
			for (var i = 0; i < htmlEntities.length; i++) {
				htmlEntities[i].style.pointerEvents = 'none';
			}
		},

		allowSlection: function() {
			var htmlEntities = document.querySelectorAll('.goo-entity');
			for (var i = 0; i < htmlEntities.length; i++) {
				htmlEntities[i].style.pointerEvents = 'none';
			}
